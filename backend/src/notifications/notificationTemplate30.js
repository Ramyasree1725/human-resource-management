/**
 * Notification Template Module 30
...
function renderTemplate30(templateKey, variables = {}) {
...
function listAvailableTemplates30() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload30(templateKey, variables, recipients = []) {
  const rendered = renderTemplate30(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate30,
  listAvailableTemplates30,
  buildNotificationPayload30
};
