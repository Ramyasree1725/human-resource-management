/**
 * Notification Template Module 11
...
function renderTemplate11(templateKey, variables = {}) {
...
function listAvailableTemplates11() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload11(templateKey, variables, recipients = []) {
  const rendered = renderTemplate11(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate11,
  listAvailableTemplates11,
  buildNotificationPayload11
};
