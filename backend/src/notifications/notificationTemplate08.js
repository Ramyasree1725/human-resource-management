/**
 * Notification Template Module 08
...
function renderTemplate8(templateKey, variables = {}) {
...
function listAvailableTemplates8() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload8(templateKey, variables, recipients = []) {
  const rendered = renderTemplate8(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate8,
  listAvailableTemplates8,
  buildNotificationPayload8
};
