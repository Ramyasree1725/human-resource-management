/**
 * Notification Template Module 32
...
function renderTemplate32(templateKey, variables = {}) {
...
function listAvailableTemplates32() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload32(templateKey, variables, recipients = []) {
  const rendered = renderTemplate32(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate32,
  listAvailableTemplates32,
  buildNotificationPayload32
};
