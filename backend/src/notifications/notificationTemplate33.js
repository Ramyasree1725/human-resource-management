/**
 * Notification Template Module 33
...
function renderTemplate33(templateKey, variables = {}) {
...
function listAvailableTemplates33() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload33(templateKey, variables, recipients = []) {
  const rendered = renderTemplate33(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate33,
  listAvailableTemplates33,
  buildNotificationPayload33
};
