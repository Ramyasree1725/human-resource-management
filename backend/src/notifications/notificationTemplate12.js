/**
 * Notification Template Module 12
...
function renderTemplate12(templateKey, variables = {}) {
...
function listAvailableTemplates12() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload12(templateKey, variables, recipients = []) {
  const rendered = renderTemplate12(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate12,
  listAvailableTemplates12,
  buildNotificationPayload12
};
