/**
 * Notification Template Module 02
...
function renderTemplate2(templateKey, variables = {}) {
...
function listAvailableTemplates2() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload2(templateKey, variables, recipients = []) {
  const rendered = renderTemplate2(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate2,
  listAvailableTemplates2,
  buildNotificationPayload2
};
