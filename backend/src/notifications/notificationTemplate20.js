/**
 * Notification Template Module 20
...
function renderTemplate20(templateKey, variables = {}) {
...
function listAvailableTemplates20() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload20(templateKey, variables, recipients = []) {
  const rendered = renderTemplate20(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate20,
  listAvailableTemplates20,
  buildNotificationPayload20
};
