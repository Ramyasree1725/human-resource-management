/**
 * Notification Template Module 03
...
function renderTemplate3(templateKey, variables = {}) {
...
function listAvailableTemplates3() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload3(templateKey, variables, recipients = []) {
  const rendered = renderTemplate3(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate3,
  listAvailableTemplates3,
  buildNotificationPayload3
};
