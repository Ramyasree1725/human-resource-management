/**
 * Notification Template Module 27
...
function renderTemplate27(templateKey, variables = {}) {
...
function listAvailableTemplates27() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload27(templateKey, variables, recipients = []) {
  const rendered = renderTemplate27(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate27,
  listAvailableTemplates27,
  buildNotificationPayload27
};
