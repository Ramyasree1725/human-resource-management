/**
 * Notification Template Module 21
...
function renderTemplate21(templateKey, variables = {}) {
...
function listAvailableTemplates21() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload21(templateKey, variables, recipients = []) {
  const rendered = renderTemplate21(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate21,
  listAvailableTemplates21,
  buildNotificationPayload21
};
