/**
 * Notification Template Module 29
...
function renderTemplate29(templateKey, variables = {}) {
...
function listAvailableTemplates29() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload29(templateKey, variables, recipients = []) {
  const rendered = renderTemplate29(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate29,
  listAvailableTemplates29,
  buildNotificationPayload29
};
