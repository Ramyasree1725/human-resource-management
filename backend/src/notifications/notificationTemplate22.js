/**
 * Notification Template Module 22
...
function renderTemplate22(templateKey, variables = {}) {
...
function listAvailableTemplates22() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload22(templateKey, variables, recipients = []) {
  const rendered = renderTemplate22(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate22,
  listAvailableTemplates22,
  buildNotificationPayload22
};
