/**
 * Notification Template Module 10
...
function renderTemplate10(templateKey, variables = {}) {
...
function listAvailableTemplates10() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload10(templateKey, variables, recipients = []) {
  const rendered = renderTemplate10(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate10,
  listAvailableTemplates10,
  buildNotificationPayload10
};
