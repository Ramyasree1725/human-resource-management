/**
 * Notification Template Module 04
...
function renderTemplate4(templateKey, variables = {}) {
...
function listAvailableTemplates4() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload4(templateKey, variables, recipients = []) {
  const rendered = renderTemplate4(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate4,
  listAvailableTemplates4,
  buildNotificationPayload4
};
