/**
 * Notification Template Module 37
...
function renderTemplate37(templateKey, variables = {}) {
...
function listAvailableTemplates37() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload37(templateKey, variables, recipients = []) {
  const rendered = renderTemplate37(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate37,
  listAvailableTemplates37,
  buildNotificationPayload37
};
