/**
 * Notification Template Module 38
...
function renderTemplate38(templateKey, variables = {}) {
...
function listAvailableTemplates38() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload38(templateKey, variables, recipients = []) {
  const rendered = renderTemplate38(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate38,
  listAvailableTemplates38,
  buildNotificationPayload38
};
