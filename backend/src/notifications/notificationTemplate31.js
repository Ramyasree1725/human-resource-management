/**
 * Notification Template Module 31
...
function renderTemplate31(templateKey, variables = {}) {
...
function listAvailableTemplates31() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload31(templateKey, variables, recipients = []) {
  const rendered = renderTemplate31(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate31,
  listAvailableTemplates31,
  buildNotificationPayload31
};
