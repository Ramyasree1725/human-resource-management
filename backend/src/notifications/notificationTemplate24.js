/**
 * Notification Template Module 24
...
function renderTemplate24(templateKey, variables = {}) {
...
function listAvailableTemplates24() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload24(templateKey, variables, recipients = []) {
  const rendered = renderTemplate24(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate24,
  listAvailableTemplates24,
  buildNotificationPayload24
};
