/**
 * Notification Template Module 44
...
function renderTemplate44(templateKey, variables = {}) {
...
function listAvailableTemplates44() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload44(templateKey, variables, recipients = []) {
  const rendered = renderTemplate44(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate44,
  listAvailableTemplates44,
  buildNotificationPayload44
};
