/**
 * Notification Template Module 01
...
function renderTemplate1(templateKey, variables = {}) {
...
function listAvailableTemplates1() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload1(templateKey, variables, recipients = []) {
  const rendered = renderTemplate1(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate1,
  listAvailableTemplates1,
  buildNotificationPayload1
};
