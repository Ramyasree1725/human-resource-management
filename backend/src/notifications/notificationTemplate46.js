/**
 * Notification Template Module 46
...
function renderTemplate46(templateKey, variables = {}) {
...
function listAvailableTemplates46() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload46(templateKey, variables, recipients = []) {
  const rendered = renderTemplate46(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate46,
  listAvailableTemplates46,
  buildNotificationPayload46
};
