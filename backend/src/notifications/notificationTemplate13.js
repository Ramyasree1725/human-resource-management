/**
 * Notification Template Module 13
...
function renderTemplate13(templateKey, variables = {}) {
...
function listAvailableTemplates13() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload13(templateKey, variables, recipients = []) {
  const rendered = renderTemplate13(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate13,
  listAvailableTemplates13,
  buildNotificationPayload13
};
