/**
 * Notification Template Module 16
...
function renderTemplate16(templateKey, variables = {}) {
...
function listAvailableTemplates16() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload16(templateKey, variables, recipients = []) {
  const rendered = renderTemplate16(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate16,
  listAvailableTemplates16,
  buildNotificationPayload16
};
