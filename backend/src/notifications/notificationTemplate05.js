/**
 * Notification Template Module 05
...
function renderTemplate5(templateKey, variables = {}) {
...
function listAvailableTemplates5() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload5(templateKey, variables, recipients = []) {
  const rendered = renderTemplate5(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate5,
  listAvailableTemplates5,
  buildNotificationPayload5
};
