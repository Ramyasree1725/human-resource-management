/**
 * Notification Template Module 18
...
function renderTemplate18(templateKey, variables = {}) {
...
function listAvailableTemplates18() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload18(templateKey, variables, recipients = []) {
  const rendered = renderTemplate18(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate18,
  listAvailableTemplates18,
  buildNotificationPayload18
};
