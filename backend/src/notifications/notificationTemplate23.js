/**
 * Notification Template Module 23
...
function renderTemplate23(templateKey, variables = {}) {
...
function listAvailableTemplates23() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload23(templateKey, variables, recipients = []) {
  const rendered = renderTemplate23(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate23,
  listAvailableTemplates23,
  buildNotificationPayload23
};
