/**
 * Notification Template Module 09
...
function renderTemplate9(templateKey, variables = {}) {
...
function listAvailableTemplates9() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload9(templateKey, variables, recipients = []) {
  const rendered = renderTemplate9(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate9,
  listAvailableTemplates9,
  buildNotificationPayload9
};
