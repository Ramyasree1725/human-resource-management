/**
 * Notification Template Module 15
...
function renderTemplate15(templateKey, variables = {}) {
...
function listAvailableTemplates15() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload15(templateKey, variables, recipients = []) {
  const rendered = renderTemplate15(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate15,
  listAvailableTemplates15,
  buildNotificationPayload15
};
