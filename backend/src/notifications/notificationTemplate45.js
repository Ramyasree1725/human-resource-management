/**
 * Notification Template Module 45
...
function renderTemplate45(templateKey, variables = {}) {
...
function listAvailableTemplates45() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload45(templateKey, variables, recipients = []) {
  const rendered = renderTemplate45(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate45,
  listAvailableTemplates45,
  buildNotificationPayload45
};
