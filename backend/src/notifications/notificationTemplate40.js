/**
 * Notification Template Module 40
...
function renderTemplate40(templateKey, variables = {}) {
...
function listAvailableTemplates40() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload40(templateKey, variables, recipients = []) {
  const rendered = renderTemplate40(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate40,
  listAvailableTemplates40,
  buildNotificationPayload40
};
