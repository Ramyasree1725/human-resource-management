/**
 * Notification Template Module 25
...
function renderTemplate25(templateKey, variables = {}) {
...
function listAvailableTemplates25() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload25(templateKey, variables, recipients = []) {
  const rendered = renderTemplate25(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate25,
  listAvailableTemplates25,
  buildNotificationPayload25
};
