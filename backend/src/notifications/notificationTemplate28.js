/**
 * Notification Template Module 28
...
function renderTemplate28(templateKey, variables = {}) {
...
function listAvailableTemplates28() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload28(templateKey, variables, recipients = []) {
  const rendered = renderTemplate28(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate28,
  listAvailableTemplates28,
  buildNotificationPayload28
};
