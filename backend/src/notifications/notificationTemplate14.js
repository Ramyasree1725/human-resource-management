/**
 * Notification Template Module 14
...
function renderTemplate14(templateKey, variables = {}) {
...
function listAvailableTemplates14() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload14(templateKey, variables, recipients = []) {
  const rendered = renderTemplate14(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate14,
  listAvailableTemplates14,
  buildNotificationPayload14
};
