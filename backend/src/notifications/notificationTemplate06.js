/**
 * Notification Template Module 06
...
function renderTemplate6(templateKey, variables = {}) {
...
function listAvailableTemplates6() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload6(templateKey, variables, recipients = []) {
  const rendered = renderTemplate6(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate6,
  listAvailableTemplates6,
  buildNotificationPayload6
};
