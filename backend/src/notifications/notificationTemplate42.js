/**
 * Notification Template Module 42
...
function renderTemplate42(templateKey, variables = {}) {
...
function listAvailableTemplates42() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload42(templateKey, variables, recipients = []) {
  const rendered = renderTemplate42(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate42,
  listAvailableTemplates42,
  buildNotificationPayload42
};
