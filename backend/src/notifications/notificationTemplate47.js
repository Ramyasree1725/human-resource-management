/**
 * Notification Template Module 47
...
function renderTemplate47(templateKey, variables = {}) {
...
function listAvailableTemplates47() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload47(templateKey, variables, recipients = []) {
  const rendered = renderTemplate47(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate47,
  listAvailableTemplates47,
  buildNotificationPayload47
};
