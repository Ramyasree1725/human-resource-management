/**
 * Notification Template Module 36
...
function renderTemplate36(templateKey, variables = {}) {
...
function listAvailableTemplates36() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload36(templateKey, variables, recipients = []) {
  const rendered = renderTemplate36(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate36,
  listAvailableTemplates36,
  buildNotificationPayload36
};
