/**
 * Notification Template Module 35
...
function renderTemplate35(templateKey, variables = {}) {
...
function listAvailableTemplates35() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload35(templateKey, variables, recipients = []) {
  const rendered = renderTemplate35(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate35,
  listAvailableTemplates35,
  buildNotificationPayload35
};
