/**
 * Notification Template Module 48
...
function renderTemplate48(templateKey, variables = {}) {
...
function listAvailableTemplates48() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload48(templateKey, variables, recipients = []) {
  const rendered = renderTemplate48(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate48,
  listAvailableTemplates48,
  buildNotificationPayload48
};
