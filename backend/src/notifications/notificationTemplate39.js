/**
 * Notification Template Module 39
...
function renderTemplate39(templateKey, variables = {}) {
...
function listAvailableTemplates39() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload39(templateKey, variables, recipients = []) {
  const rendered = renderTemplate39(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate39,
  listAvailableTemplates39,
  buildNotificationPayload39
};
