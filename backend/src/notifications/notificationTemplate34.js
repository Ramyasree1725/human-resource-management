/**
 * Notification Template Module 34
...
function renderTemplate34(templateKey, variables = {}) {
...
function listAvailableTemplates34() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload34(templateKey, variables, recipients = []) {
  const rendered = renderTemplate34(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate34,
  listAvailableTemplates34,
  buildNotificationPayload34
};
