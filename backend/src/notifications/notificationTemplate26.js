/**
 * Notification Template Module 26
...
function renderTemplate26(templateKey, variables = {}) {
...
function listAvailableTemplates26() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload26(templateKey, variables, recipients = []) {
  const rendered = renderTemplate26(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate26,
  listAvailableTemplates26,
  buildNotificationPayload26
};
