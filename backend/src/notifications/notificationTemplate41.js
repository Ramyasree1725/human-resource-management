/**
 * Notification Template Module 41
...
function renderTemplate41(templateKey, variables = {}) {
...
function listAvailableTemplates41() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload41(templateKey, variables, recipients = []) {
  const rendered = renderTemplate41(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate41,
  listAvailableTemplates41,
  buildNotificationPayload41
};
