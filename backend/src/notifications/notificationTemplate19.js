/**
 * Notification Template Module 19
...
function renderTemplate19(templateKey, variables = {}) {
...
function listAvailableTemplates19() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload19(templateKey, variables, recipients = []) {
  const rendered = renderTemplate19(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate19,
  listAvailableTemplates19,
  buildNotificationPayload19
};
