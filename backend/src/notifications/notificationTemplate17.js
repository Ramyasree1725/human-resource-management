/**
 * Notification Template Module 17
...
function renderTemplate17(templateKey, variables = {}) {
...
function listAvailableTemplates17() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload17(templateKey, variables, recipients = []) {
  const rendered = renderTemplate17(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate17,
  listAvailableTemplates17,
  buildNotificationPayload17
};
