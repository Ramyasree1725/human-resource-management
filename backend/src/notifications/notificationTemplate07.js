/**
 * Notification Template Module 07
...
function renderTemplate7(templateKey, variables = {}) {
...
function listAvailableTemplates7() {
  return Object.keys(TEMPLATES).map(k => ({ key: k, subject: TEMPLATES[k].subject }));
}

function buildNotificationPayload7(templateKey, variables, recipients = []) {
  const rendered = renderTemplate7(templateKey, variables);
  return {
    ...rendered,
    recipients,
    channel: "email",
...
module.exports = {
  TEMPLATES,
  renderTemplate7,
  listAvailableTemplates7,
  buildNotificationPayload7
};
