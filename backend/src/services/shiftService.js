/**
 * Shift Service
 * Business logic layer for shift management in the Employee Management System.
...
class ShiftService {
  constructor() {
    this.entityName = 'Shift';
    this.cache = new Map();
    this.cacheTTL = 5 * 60 * 1000; // 5 minutes
  }

  /**
   * Retrieve all shift records with optional filtering, sorting and pagination.
  ...
  /**
   * Find a single shift by its unique identifier.
   */
  async findById(id) {
    ...
    if (!record) {
      return { success: false, message: 'Shift not found', statusCode: 404 };
    }
  ...
  /**
   * Create a new shift record after running full validation and business rules.
   */
  ...
  /**
   * Soft or hard delete a shift record.
   */
