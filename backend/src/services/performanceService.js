/**
 * Performance Service
 * Business logic layer for performance management in the Employee Management System.
...
class PerformanceService {
  constructor() {
    this.entityName = 'Performance';
    this.cache = new Map();
    this.cacheTTL = 5 * 60 * 1000; // 5 minutes
  }

  /**
   * Retrieve all performance records with optional filtering, sorting and pagination.
  ...
  /**
   * Find a single performance by its unique identifier.
   */
  async findById(id) {
    ...
    if (!record) {
      return { success: false, message: 'Performance not found', statusCode: 404 };
    }
  ...
  /**
   * Create a new performance record after running full validation and business rules.
   */
  ...
  /**
   * Soft or hard delete a performance record.
   */
  async remove(id, actor = null, hard = false) {
    ...
    return { success: true, message: 'Performance removed successfully' };
  }
