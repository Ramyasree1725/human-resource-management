...
function computeMetric15(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 15;
    sum += val;
    count += 1;
...
function aggregateByDepartment15(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule15"
  }));
}

function runFullAnalysis15(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric15(employees, options);
  const byDept = aggregateByDepartment15(employees);
  return {
    analysisId: "analysis_15_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
