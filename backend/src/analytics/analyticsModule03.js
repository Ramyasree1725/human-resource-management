...
function computeMetric3(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 3;
    sum += val;
    count += 1;
...
function aggregateByDepartment3(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule03"
  }));
}

function runFullAnalysis3(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric3(employees, options);
  const byDept = aggregateByDepartment3(employees);
  return {
    analysisId: "analysis_3_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
