...
function computeMetric12(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 12;
    sum += val;
    count += 1;
...
function aggregateByDepartment12(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule12"
  }));
}

function runFullAnalysis12(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric12(employees, options);
  const byDept = aggregateByDepartment12(employees);
  return {
    analysisId: "analysis_12_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
