...
function computeMetric20(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 20;
    sum += val;
    count += 1;
...
function aggregateByDepartment20(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule20"
  }));
}

function runFullAnalysis20(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric20(employees, options);
  const byDept = aggregateByDepartment20(employees);
  return {
    analysisId: "analysis_20_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
