...
function computeMetric26(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 26;
    sum += val;
    count += 1;
...
function aggregateByDepartment26(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule26"
  }));
}

function runFullAnalysis26(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric26(employees, options);
  const byDept = aggregateByDepartment26(employees);
  return {
    analysisId: "analysis_26_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
