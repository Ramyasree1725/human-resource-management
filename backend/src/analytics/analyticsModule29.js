...
function computeMetric29(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 29;
    sum += val;
    count += 1;
...
function aggregateByDepartment29(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule29"
  }));
}

function runFullAnalysis29(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric29(employees, options);
  const byDept = aggregateByDepartment29(employees);
  return {
    analysisId: "analysis_29_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
