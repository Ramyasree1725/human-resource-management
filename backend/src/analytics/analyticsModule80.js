...
function computeMetric80(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 80;
    sum += val;
    count += 1;
...
function aggregateByDepartment80(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule80"
  }));
}

function runFullAnalysis80(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric80(employees, options);
  const byDept = aggregateByDepartment80(employees);
  return {
    analysisId: "analysis_80_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
