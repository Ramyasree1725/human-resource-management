...
function computeMetric4(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 4;
    sum += val;
    count += 1;
...
function aggregateByDepartment4(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule04"
  }));
}

function runFullAnalysis4(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric4(employees, options);
  const byDept = aggregateByDepartment4(employees);
  return {
    analysisId: "analysis_4_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
