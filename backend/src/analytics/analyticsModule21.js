...
function computeMetric21(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 21;
    sum += val;
    count += 1;
...
function aggregateByDepartment21(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule21"
  }));
}

function runFullAnalysis21(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric21(employees, options);
  const byDept = aggregateByDepartment21(employees);
  return {
    analysisId: "analysis_21_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
