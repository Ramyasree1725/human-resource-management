...
function computeMetric40(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 40;
    sum += val;
    count += 1;
...
function aggregateByDepartment40(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule40"
  }));
}

function runFullAnalysis40(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric40(employees, options);
  const byDept = aggregateByDepartment40(employees);
  return {
    analysisId: "analysis_40_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
