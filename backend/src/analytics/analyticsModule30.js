...
function computeMetric30(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 30;
    sum += val;
    count += 1;
...
function aggregateByDepartment30(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule30"
  }));
}

function runFullAnalysis30(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric30(employees, options);
  const byDept = aggregateByDepartment30(employees);
  return {
    analysisId: "analysis_30_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
