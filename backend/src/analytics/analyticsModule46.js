...
function computeMetric46(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 46;
    sum += val;
    count += 1;
...
function aggregateByDepartment46(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule46"
  }));
}

function runFullAnalysis46(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric46(employees, options);
  const byDept = aggregateByDepartment46(employees);
  return {
    analysisId: "analysis_46_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
