...
function computeMetric25(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 25;
    sum += val;
    count += 1;
...
function aggregateByDepartment25(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule25"
  }));
}

function runFullAnalysis25(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric25(employees, options);
  const byDept = aggregateByDepartment25(employees);
  return {
    analysisId: "analysis_25_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
