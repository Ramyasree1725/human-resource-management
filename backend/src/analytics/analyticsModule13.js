...
function computeMetric13(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 13;
    sum += val;
    count += 1;
...
function aggregateByDepartment13(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule13"
  }));
}

function runFullAnalysis13(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric13(employees, options);
  const byDept = aggregateByDepartment13(employees);
  return {
    analysisId: "analysis_13_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
