...
function computeMetric41(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 41;
    sum += val;
    count += 1;
...
function aggregateByDepartment41(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule41"
  }));
}

function runFullAnalysis41(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric41(employees, options);
  const byDept = aggregateByDepartment41(employees);
  return {
    analysisId: "analysis_41_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
