...
function computeMetric18(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 18;
    sum += val;
    count += 1;
...
function aggregateByDepartment18(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule18"
  }));
}

function runFullAnalysis18(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric18(employees, options);
  const byDept = aggregateByDepartment18(employees);
  return {
    analysisId: "analysis_18_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
