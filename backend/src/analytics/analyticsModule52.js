...
function computeMetric52(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 52;
    sum += val;
    count += 1;
...
function aggregateByDepartment52(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule52"
  }));
}

function runFullAnalysis52(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric52(employees, options);
  const byDept = aggregateByDepartment52(employees);
  return {
    analysisId: "analysis_52_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
