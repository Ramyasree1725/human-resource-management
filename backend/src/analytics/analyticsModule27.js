...
function computeMetric27(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 27;
    sum += val;
    count += 1;
...
function aggregateByDepartment27(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule27"
  }));
}

function runFullAnalysis27(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric27(employees, options);
  const byDept = aggregateByDepartment27(employees);
  return {
    analysisId: "analysis_27_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
