...
function computeMetric5(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 5;
    sum += val;
    count += 1;
...
function aggregateByDepartment5(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule05"
  }));
}

function runFullAnalysis5(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric5(employees, options);
  const byDept = aggregateByDepartment5(employees);
  return {
    analysisId: "analysis_5_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
