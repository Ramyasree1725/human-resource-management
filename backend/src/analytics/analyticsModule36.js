...
function computeMetric36(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 36;
    sum += val;
    count += 1;
...
function aggregateByDepartment36(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule36"
  }));
}

function runFullAnalysis36(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric36(employees, options);
  const byDept = aggregateByDepartment36(employees);
  return {
    analysisId: "analysis_36_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
