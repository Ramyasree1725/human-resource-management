...
function computeMetric10(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 10;
    sum += val;
    count += 1;
...
function aggregateByDepartment10(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule10"
  }));
}

function runFullAnalysis10(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric10(employees, options);
  const byDept = aggregateByDepartment10(employees);
  return {
    analysisId: "analysis_10_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
