...
function computeMetric11(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 11;
    sum += val;
    count += 1;
...
function aggregateByDepartment11(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule11"
  }));
}

function runFullAnalysis11(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric11(employees, options);
  const byDept = aggregateByDepartment11(employees);
  return {
    analysisId: "analysis_11_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
