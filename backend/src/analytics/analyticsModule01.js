...
function computeMetric1(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 1;
    sum += val;
    count += 1;
...
function aggregateByDepartment1(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule01"
  }));
}

function runFullAnalysis1(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric1(employees, options);
  const byDept = aggregateByDepartment1(employees);
  return {
    analysisId: "analysis_1_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
