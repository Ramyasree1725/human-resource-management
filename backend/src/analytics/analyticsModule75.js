...
function computeMetric75(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 75;
    sum += val;
    count += 1;
...
function aggregateByDepartment75(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule75"
  }));
}

function runFullAnalysis75(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric75(employees, options);
  const byDept = aggregateByDepartment75(employees);
  return {
    analysisId: "analysis_75_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
