...
function computeMetric2(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 2;
    sum += val;
    count += 1;
...
function aggregateByDepartment2(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule02"
  }));
}

function runFullAnalysis2(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric2(employees, options);
  const byDept = aggregateByDepartment2(employees);
  return {
    analysisId: "analysis_2_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
