...
function computeMetric14(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 14;
    sum += val;
    count += 1;
...
function aggregateByDepartment14(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule14"
  }));
}

function runFullAnalysis14(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric14(employees, options);
  const byDept = aggregateByDepartment14(employees);
  return {
    analysisId: "analysis_14_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
