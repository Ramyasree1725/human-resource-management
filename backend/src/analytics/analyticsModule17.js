...
function computeMetric17(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 17;
    sum += val;
    count += 1;
...
function aggregateByDepartment17(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule17"
  }));
}

function runFullAnalysis17(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric17(employees, options);
  const byDept = aggregateByDepartment17(employees);
  return {
    analysisId: "analysis_17_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
