...
function computeMetric37(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 37;
    sum += val;
    count += 1;
...
function aggregateByDepartment37(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule37"
  }));
}

function runFullAnalysis37(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric37(employees, options);
  const byDept = aggregateByDepartment37(employees);
  return {
    analysisId: "analysis_37_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
