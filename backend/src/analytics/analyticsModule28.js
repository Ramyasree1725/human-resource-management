...
function computeMetric28(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 28;
    sum += val;
    count += 1;
...
function aggregateByDepartment28(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule28"
  }));
}

function runFullAnalysis28(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric28(employees, options);
  const byDept = aggregateByDepartment28(employees);
  return {
    analysisId: "analysis_28_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
