...
function computeMetric58(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 58;
    sum += val;
    count += 1;
...
function aggregateByDepartment58(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule58"
  }));
}

function runFullAnalysis58(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric58(employees, options);
  const byDept = aggregateByDepartment58(employees);
  return {
    analysisId: "analysis_58_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
