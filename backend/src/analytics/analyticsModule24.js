...
function computeMetric24(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 24;
    sum += val;
    count += 1;
...
function aggregateByDepartment24(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule24"
  }));
}

function runFullAnalysis24(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric24(employees, options);
  const byDept = aggregateByDepartment24(employees);
  return {
    analysisId: "analysis_24_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
