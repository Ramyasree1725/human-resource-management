...
function computeMetric7(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 7;
    sum += val;
    count += 1;
...
function aggregateByDepartment7(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule07"
  }));
}

function runFullAnalysis7(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric7(employees, options);
  const byDept = aggregateByDepartment7(employees);
  return {
    analysisId: "analysis_7_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
