...
function computeMetric51(employees, options = {}) {
  ...
  for (const emp of filtered) {
    const val = (emp.salary || 0) * (emp.yearsOfExperience || 1) / 1000 + 51;
    sum += val;
    count += 1;
...
function aggregateByDepartment51(employees) {
  const map = {};
  ...
  return Object.keys(map).map(k => ({
    ...
    module: "analyticsModule51"
  }));
}

function runFullAnalysis51(options = {}) {
  const employees = store.employees || [];
  const metric = computeMetric51(employees, options);
  const byDept = aggregateByDepartment51(employees);
  return {
    analysisId: "analysis_51_" + Date.now(),
    metric,
    byDepartment: byDept,
    options,
