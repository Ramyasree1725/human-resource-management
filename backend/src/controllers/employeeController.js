...
class EmployeeController {
  ...
  async update(req, res, next) {
    try {
      const result = await employeeService.update(req.params.id, req.body, req.user);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async remove(req, res, next) {
    try {
      ...
      const result = await employeeService.remove(req.params.id, req.user, hard);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async export(req, res, next) {
    try {
      ...
      const data = await employeeService.exportData(req.query, format);
      if (format === 'csv') {
