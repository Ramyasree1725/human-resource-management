...
class AttendanceController {
  ...
  async create(req, res, next) {
    try {
      const result = await attendanceService.create(req.body, req.user);
  ...
  async update(req, res, next) {
    try {
      const result = await attendanceService.update(req.params.id, req.body, req.user);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async remove(req, res, next) {
    try {
      ...
      const result = await attendanceService.remove(req.params.id, req.user, hard);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async export(req, res, next) {
    try {
      ...
      const data = await attendanceService.exportData(req.query, format);
      if (format === 'csv') {
