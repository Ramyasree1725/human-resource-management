/**
 * Leave Controller
 * HTTP request handlers for leave endpoints.
...
class LeaveController {
  async list(req, res, next) {
    try {
      const result = await leaveService.findAll(req.query);
  ...
  async remove(req, res, next) {
    try {
      ...
      const result = await leaveService.remove(req.params.id, req.user, hard);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async export(req, res, next) {
    try {
      ...
      const data = await leaveService.exportData(req.query, format);
      if (format === 'csv') {
