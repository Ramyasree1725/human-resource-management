...
class ReportController {
  async list(req, res, next) {
    try {
      const result = await reportService.findAll(req.query);
  ...
  async getById(req, res, next) {
    try {
      const result = await reportService.findById(req.params.id);
      if (!result.success) {
  ...
  async remove(req, res, next) {
    try {
      ...
      const result = await reportService.remove(req.params.id, req.user, hard);
      if (!result.success) {
        return res.status(result.statusCode || 404).json(result);
      }
      res.status(200).json(result);
  ...
  async export(req, res, next) {
    try {
      ...
      const data = await reportService.exportData(req.query, format);
      if (format === 'csv') {
